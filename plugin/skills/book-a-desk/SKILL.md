...
2. **Resolve the location.** If the user named a building you already have an id for, or `whoami` returned a `homeLocationId` that matches what they mean, skip this. Otherwise call `list_locations` with `city`, `query`, or `lat`/`lng` + `radius_km` and pick from the results. If several buildings plausibly match, ask rather than guess.
3. **Search.** Call `search_availability` with `location_id` (or `city` to sweep a whole city), `date`, and `start_time`/`end_time` in the building's local time. Default to a full working day only if the user did not say; if they said "the morning", use 09:00-13:00. Each result carries an opaque signed `quote`.
4. **Show the user the options.** Present at most three to five, each with: building name, space name, the date and **local** start/end times, the **credit cost**, and seats remaining. Never present a booking option without its credit cost.
...
- **Never book without showing the credit cost and getting a yes.** No exceptions, no "I assumed", no booking as a side effect of another request.
