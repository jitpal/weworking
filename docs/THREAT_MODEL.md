...
3. **WeWork.** Treated as hostile-by-accident: responses are parsed defensively, HTTP 200 is not taken as success (`BookingStatus` is checked), and nothing from upstream is interpolated into HTML without escaping. The Auth0 flows go further and treat upstream as a source of destinations: the tenant `domain` and `redirect_uri` that discovery returns are accepted only when the host is `wework.com`, a subdomain of it, or a subdomain of `auth0.com` (otherwise the pinned constants are used), and the headless login refuses to follow a redirect or submit a form to any other host. That is what keeps `WEWORK_USERNAME`, `WEWORK_PASSWORD` and the single-use `login_ticket` (which travels as a `Referer`) on WeWork's own infrastructure.
...
- All logging goes through `src/redact.ts`. `redact()` replaces the value of any property whose *name* contains `token`, `password`, `secret`, `authorization`, `weworkauth`, `cookie`, `refresh` or `email`; `redactHeaders()` applies the same test to header names; `redactUrl()` also replaces the `code`, `state` and `login_ticket` query parameters. On top of that one value-level rule masks anything shaped like an email address inside a string, wherever it appears, including free text and an `Error` message. It is otherwise a key-name filter and not a content scanner, so the thing that keeps a profile body or an upstream response out of the log stream is still that no call site logs one: what gets logged is a label, a status, a byte count and an error code.
...
- **`WeWorkUUID`** and the user's email are treated as personal data. They appear in `whoami` output, because the user asked for it, and no log line includes them: an `email` key and anything shaped like an address in a string value are both replaced by `redact()`.
