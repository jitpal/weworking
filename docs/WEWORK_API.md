...
- **`get-locations-by-geo` item:** `{ uuid, name, latitude, longitude, address: { line1, line2, city, state, country, zip }, timeZone, distance, brandName, accountType, spaceAvailabilityCount, ... }`. On a city search `address.country` is empty and `distance` is meaningless (no origin), so both are dropped. There is no offset or currency field; derive the offset from `timeZone` and take the currency from the `get-spaces` location instead.
...
- **`get-spaces` `locationOffset` must match the building.** Sending `+00:00` for a New York building returned `totalCount: 0` while the correct `-04:00` returned inventory ($39.00 day rate, USD). London tolerated `+00:00` only because it is within the same day. Building metadata is therefore persisted in the Durable Object so a search by bare `location_id` can send the right offset in any isolate; an id never seen before is queried at `+00:00` and, if that reveals a building in another zone, queried again at that offset.
- **Tokyo (`accountType` 4) returned `totalCount: 0`** for this account on a city sweep with the correct `+09:00` offset, so that is inventory or eligibility, not a request bug. Berlin (EUR) and Sydney (AUD) priced correctly, including a Sydney start that falls on the previous UTC date.
- **`distance` on `get-locations-by-geo` is metres** (327.37 for a building 330 m away); our own great-circle figure is used when a search origin exists.
