...
3. **A building we have never seen is still searchable.** The first request goes out with `+00:00`; if the results reveal a building in another zone, the request is repeated with that zone's offset. If the first pass is empty nothing is revealed, and the response says so and suggests a city search. `list_locations` before `search_availability` is the reliable order.
4. **City searches sweep at most ten buildings** per call, in the order WeWork returns them. Coordinates searches use the same fixed bounding box the web app sends and filter by our own great-circle distance in kilometres. WeWork's own `distance` field is in metres and, on a city search, measured from nowhere useful, so it is ignored.
...
13. **Quotes are short-lived** (`QUOTE_TTL_SECONDS`, default 600) and expire on the UTC clock. An expired quote is refused; the agent searches again.

## What this means for an agent

- Ask for a building by id from `list_locations`, or search a whole city.
